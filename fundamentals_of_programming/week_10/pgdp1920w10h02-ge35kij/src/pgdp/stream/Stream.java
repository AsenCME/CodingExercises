...
public interface Stream<T> {
    ...
    static <T> Stream<T> of(java.util.stream.Stream<T> javaStream) {
        if (javaStream == null)
            throw new NullPointerException("Collections cannot bu null.");
        return new SourcePart<T>(StreamIterator.of(javaStream),
                StreamCharacteristics.REGULAR.withStreamSize(javaStream.count()));
    }
    ...
    static <T> Stream<T> of(Collection<T> col) {
        if (col == null)
            throw new NullPointerException("Collections cannot bu null.");
        return new SourcePart<T>(StreamIterator.of(col), StreamCharacteristics.REGULAR.withStreamSize(col.size()));
    }
    ...
    static <T> Stream<T> of(T... elements) {
        if (elements == null)
            throw new NullPointerException("Collections cannot bu null.");
        return new SourcePart<T>(StreamIterator.of(elements),
                StreamCharacteristics.REGULAR.withStreamSize(elements.length));
    }
}
